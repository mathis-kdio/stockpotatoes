...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formapparition"))
{
  ...
  $updateSQL = sprintf("UPDATE stock_theme SET date_apparition =%s WHERE ID_theme ='%s' ", GetSQLValueString($testdateupdate, "date"), $testthemeupdate);
...
if ((isset($_POST["MM_update2"])) && ($_POST["MM_update2"] == "formdisparition"))
{
  ...
  $updateSQL2 = sprintf("UPDATE stock_theme SET date_disparition =%s WHERE ID_theme ='%s' ", GetSQLValueString($testdateupdate2, "date"), $testthemeupdate2);
  mysqli_select_db($conn_intranet, $database_conn_intranet);
